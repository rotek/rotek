var basePagingGrid = function (gridConfig)
{
	...
	var storeURL = gridConfig.storeURL;
	...
	var pagesize = gridConfig.pagesize?gridConfig.pagesize:10;
	...
	var store = this.setStore (storeURL,pagesize,gridConfig.gridId,baseParams);
	this.store = store;
	...
	basePagingGrid.superclass.constructor.call(this, {
			id:id?id:"",
			store: store,
...
Ext.extend(basePagingGrid, Ext.grid.GridPanel, {
	/**
	 * 设置列和读取器
	 * */
	setBase:function(gridCmReaderConfig,sm)
	{
		...
		for(var i=0;i<gridCmReaderConfig.length;i++)
		{
		    ...
		    if (oname.editor) cmobj.editor = oname.editor;
		    if (oname.renderer) cmobj.renderer = oname.renderer;
		    if (oname.sortable)  cmobj.sortable = true;
	...
	/**设置存储器****/
	setStore :function (storeURL,pagesize,gridId,baseParams)
	{

   	   var store2 = new Ext.data.Store({
	        proxy: new Ext.data.HttpProxy({url:storeURL}),
    	...
    	store2.load({params:{start:0,limit:pagesize?pagesize:10}});
		return store2;
	},
	/***/
	delRows :function (records)
	{
		if(records)
		{
			...
			if(this.pages)
			{
				 ...
				 this.pagebar.updateInfo();//更改工具条显示信息
	...
	addRows:function(records)
	{
		...
		{
			var count = 0;
			if(typeof(records.length)=='number')//获得新增的条数
			{
				count = records.length;
			}
			else
