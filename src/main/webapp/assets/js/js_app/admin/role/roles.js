...
if (toolbar.get("button_modify")) {
	toolbar
			...
			.setHandler(
					function() {
						var selections = gridPanel.getSelectionModel()
								.getSelections();
														...
														"checkchange" : function(
															...
															if (node.attributes.leaf) {
																...
																if (node.childNodes.length > 0
																		&& !node.childNodes[0].attributes.leaf) {
																	return;
																}
																Ext
																		.each(
																				...
																				function(
																					...
																					child
																							.getUI().checkbox.checked = checked;
																				});
															}
														}
													}
										...
										var updateWindow = new CTA.common.UpdateWindow(
												{
													...
													items : [ tabs ],
													handler : function() {
														if (role_info_formPanel
																.getForm()
																.isValid()) {
															var params = role_info_formPanel
																	.getForm()
																	.getValues();
															var menuList = [];
															var nodeList = role_info_treePanel
																	.getChecked();
...
if (toolbar.get("button_drop")) {
	toolbar.get("button_drop").setHandler(
			function() {
				...
				Ext.Msg.confirm("警告", "您确定删除这  " + ids.length + " 条数据吗？",
						function(button) {
							if ('yes' == button) {
								CTA.common.Mask.showMask();
