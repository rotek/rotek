/**  管理软化器组信息  */
Ext.ns("ROTEK.COMPONENT.SOFTENER");
ROTEK.COMPONENT.SOFTENER.params = {
	gridParam : {
		url : basePath + "/admin/componentgroup/listComGroup/4",
		dataList : [ {
			index : 'id',
			header : '组ID',
			width : 30,
	...
	url : {
		addUrl : basePath + "/admin/componentgroup/addComGroup/4",
		detailUrl : basePath + "/admin/componentgroup/getComGroupDetail",
		modifyUrl : basePath + "/admin/componentgroup/modifyComGroup/4",
		dropUrl : basePath + "/admin/componentgroup/deleteComGroup",
		listProejctUrl : basePath + "/admin/componentgroup/listProjectByStatus"
	}
};

var gridPanel = CTA.common.GridPanel.createGridPanel(ROTEK.COMPONENT.SOFTENER.params.gridParam);
...
//				});
				formPanel.commit({
					url : ROTEK.COMPONENT.SOFTENER.params.url.addUrl
				});
			}
		};
		//定义添加的窗口
			...
			items : [{
    			...
    			store : new Ext.data.Store({
    				...
    				proxy : new Ext.data.HttpProxy({
    					url : ROTEK.COMPONENT.SOFTENER.params.url.listProejctUrl
    				})
    			})
    		}, {
				fieldLabel : '组编号',
...
if(toolbar.get("button_modify")){
	toolbar.get("button_modify").setHandler(function() {
		...
		Ext.Ajax.request({
			url : ROTEK.COMPONENT.SOFTENER.params.url.detailUrl,
			params : {
				id : id
			},
				...
				var updateWindow = new CTA.common.UpdateWindow({
					...
					handler : function() {
						...
						if (formPanel.getForm().isValid()) {
							...
							formPanel.commit({
								url : ROTEK.COMPONENT.SOFTENER.params.url.modifyUrl
							});
						}
					}
				});
		...
		Ext.Msg.confirm("警告", "您确定删除这  " + ids.length + " 条数据吗？", function(button) {
			if ('yes' == button) {
				...
				CTA.common.Ajax.request({
					url : ROTEK.COMPONENT.SOFTENER.params.url.dropUrl,
					params : {
						ids : ids.toString()
					}
				});
