/** 管理 膜组零件 信息  */
Ext.ns("ROTEK.COMPONENTD.FILM");
ROTEK.COMPONENTD.FILM.params = {
	gridParam : {
		url : basePath + "/admin/componentdetail/listComDetail/6/1",
		dataList : [ {
			index : 'id',
			header : '零件ID',
			width : 20,
	...
	url : {
		addUrl : basePath + "/admin/componentdetail/addComDetail/6",
		detailUrl : basePath + "/admin/componentdetail/getComDetail",
		modifyUrl : basePath + "/admin/componentdetail/modifyComDetail/6",
...
var gridPanel = CTA.common.GridPanel.createGridPanel(ROTEK.COMPONENTD.FILM.params.gridParam);
	...
	toolbar.get("button_add").setHandler(function() {
		var saveHandler = function() {
			...
			if (formPanel.getForm().isValid()) {
				formPanel.commit({
					url : ROTEK.COMPONENTD.FILM.params.url.addUrl
				});
			}
		};
		//定义添加的窗口
		...
		var ProjectStore = new Ext.data.Store({
			...
			proxy : new Ext.data.HttpProxy({
				url : ROTEK.COMPONENTD.FILM.params.url.listProejctUrl
			})
		})
...
if(toolbar.get("button_modify")){
	toolbar.get("button_modify").setHandler(function() {
		...
		Ext.Ajax.request({
			url : ROTEK.COMPONENTD.FILM.params.url.detailUrl,
			params : {
				id : id
			},
			success : function(response) {
				...
				var formPanel = new CTA.common.SFormPanel({
					...
					items : [{
						...
						allowBlank : true,
						width : 150,
						value : new Date(parseFloat(data.edqxsj)).format("Y-m-d")
					}, {
						xtype : 'numberfield',
						fieldLabel : '额定清洗压差',
						emptyText : '请输入额定清洗压差',
				...
				var updateWindow = new CTA.common.UpdateWindow({
					...
					handler : function() {
						...
						if (formPanel.getForm().isValid()) {
							...
							formPanel.commit({
								url : ROTEK.COMPONENTD.FILM.params.url.modifyUrl
							});
						}
					}
				});
		...
		Ext.Msg.confirm("警告", "您确定删除这  " + ids.length + " 条数据吗？", function(button) {
			if ('yes' == button) {
				...
				CTA.common.Ajax.request({
					url : ROTEK.COMPONENTD.FILM.params.url.dropUrl,
					params : {
						ids : ids.toString()
					}
				});
