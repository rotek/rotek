...
ROTEK.COMPONENTD.SANDFILTER.params = {
	...
	url : {
		addUrl : basePath + "/admin/componentdetail/addComDetail/2",
		detailUrl : basePath + "/admin/componentdetail/getComDetail",
		modifyUrl : basePath + "/admin/componentdetail/modifyComDetail/2",
...
var gridPanel = CTA.common.GridPanel.createGridPanel(ROTEK.COMPONENTD.SANDFILTER.params.gridParam);
	...
	toolbar.get("button_add").setHandler(function() {
		var saveHandler = function() {
			...
			if (formPanel.getForm().isValid()) {
				formPanel.commit({
					url : ROTEK.COMPONENTD.SANDFILTER.params.url.addUrl
				});
			}
		};
		//定义添加的窗口
		...
		var ProjectStore = new Ext.data.Store({
			...
			proxy : new Ext.data.HttpProxy({
				url : ROTEK.COMPONENTD.SANDFILTER.params.url.listProejctUrl
			})
		})
...
if(toolbar.get("button_modify")){
	toolbar.get("button_modify").setHandler(function() {
		...
		Ext.Ajax.request({
			url : ROTEK.COMPONENTD.SANDFILTER.params.url.detailUrl,
			params : {
				id : id
			},
				...
				var updateWindow = new CTA.common.UpdateWindow({
					...
					handler : function() {
						...
						if (formPanel.getForm().isValid()) {
							...
							formPanel.commit({
								url : ROTEK.COMPONENTD.SANDFILTER.params.url.modifyUrl
							});
						}
					}
				});
		...
		Ext.Msg.confirm("警告", "您确定删除这  " + ids.length + " 条数据吗？", function(button) {
			if ('yes' == button) {
				...
				CTA.common.Ajax.request({
					url : ROTEK.COMPONENTD.SANDFILTER.params.url.dropUrl,
					params : {
						ids : ids.toString()
					}
				});
