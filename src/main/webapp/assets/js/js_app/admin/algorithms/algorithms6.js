/** 设置算法 管理  */
Ext.ns("ROTEK.ALGORITHMS6");
ROTEK.ALGORITHMS6.params = {
	gridParam : {
		url : basePath + "/admin/algorithms/listAlgorithms/6",
		dataList : [ {
			index : 'id',
			header : 'ID',
			width : 20,
	...
	url : {
		addUrl : basePath + "/admin/algorithms/addAlgorithms/6",
		detailUrl : basePath + "/admin/algorithms/getAlgorithmDetail",
		modifyUrl : basePath + "/admin/algorithms/modifyAlgorithms/6",
...
var gridPanel = CTA.common.GridPanel.createGridPanel(ROTEK.ALGORITHMS6.params.gridParam);
	...
	toolbar.get("button_add").setHandler(function() {
		var saveHandler = function() {
			...
			if (formPanel.getForm().isValid()) {
				formPanel.commit({
					url : ROTEK.ALGORITHMS6.params.url.addUrl
				});
			}
		};
		//定义添加的窗口
		...
		var ProjectStore = new Ext.data.Store({
			...
			proxy : new Ext.data.HttpProxy({
				url : ROTEK.ALGORITHMS6.params.url.listProejctUrl
			})
		})
...
if(toolbar.get("button_modify")){
	toolbar.get("button_modify").setHandler(function() {
		...
		Ext.Ajax.request({
			url : ROTEK.ALGORITHMS6.params.url.detailUrl,
			params : {
				id : id
			},
				...
				var updateWindow = new CTA.common.UpdateWindow({
					...
					handler : function() {
						...
						if (formPanel.getForm().isValid()) {
							...
							formPanel.commit({
								url : ROTEK.ALGORITHMS6.params.url.modifyUrl
							});
						}
					}
				});
	...
	toolbar.get("button_algorithmDesc").setHandler(function() {
		var formPanel = new CTA.common.SFormPanel({
			items : [ {
				xtype : 'tbtext',
				readOnly : true,
				text : '1. 当月使用量：月累计运行时间*单位运行用量设定值S g/h，其中，月累计运行时间通过运行的开关量信息的累计运行时间计算。<p>&nbsp;</p>'
		...
		Ext.Msg.confirm("警告", "您确定删除这  " + ids.length + " 条数据吗？", function(button) {
			if ('yes' == button) {
				...
				CTA.common.Ajax.request({
					url : ROTEK.ALGORITHMS6.params.url.dropUrl,
					params : {
						ids : ids.toString()
					}
				});
