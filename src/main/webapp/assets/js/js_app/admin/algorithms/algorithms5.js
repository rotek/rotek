/** 设置算法 管理  */
Ext.ns("ROTEK.ALGORITHMS5");
ROTEK.ALGORITHMS5.params = {
	gridParam : {
		url : basePath + "/admin/algorithms/listAlgorithms/5",
		dataList : [ {
			index : 'id',
			header : 'ID',
			width : 20,
	...
	url : {
		addUrl : basePath + "/admin/algorithms/addAlgorithms/5",
		detailUrl : basePath + "/admin/algorithms/getAlgorithmDetail",
		modifyUrl : basePath + "/admin/algorithms/modifyAlgorithms/5",
...
var gridPanel = CTA.common.GridPanel.createGridPanel(ROTEK.ALGORITHMS5.params.gridParam);
	...
	toolbar.get("button_add").setHandler(function() {
		var saveHandler = function() {
			...
			if (formPanel.getForm().isValid()) {
				formPanel.commit({
					url : ROTEK.ALGORITHMS5.params.url.addUrl
				});
			}
		};
		//定义添加的窗口
		...
		var ProjectStore = new Ext.data.Store({
			...
			proxy : new Ext.data.HttpProxy({
				url : ROTEK.ALGORITHMS5.params.url.listProejctUrl
			})
		})
...
if(toolbar.get("button_modify")){
	toolbar.get("button_modify").setHandler(function() {
		...
		Ext.Ajax.request({
			url : ROTEK.ALGORITHMS5.params.url.detailUrl,
			params : {
				id : id
			},
				...
				var updateWindow = new CTA.common.UpdateWindow({
					...
					handler : function() {
						...
						if (formPanel.getForm().isValid()) {
							...
							formPanel.commit({
								url : ROTEK.ALGORITHMS5.params.url.modifyUrl
							});
						}
					}
				});
		...
		Ext.Msg.confirm("警告", "您确定删除这  " + ids.length + " 条数据吗？", function(button) {
			if ('yes' == button) {
				...
				CTA.common.Ajax.request({
					url : ROTEK.ALGORITHMS5.params.url.dropUrl,
					params : {
						ids : ids.toString()
					}
				});
