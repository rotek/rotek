/** 设置算法 管理  */
Ext.ns("ROTEK.ALGORITHMS8");
ROTEK.ALGORITHMS8.params = {
	gridParam : {
		url : basePath + "/admin/algorithms/listAlgorithms/8",
		dataList : [ {
			index : 'id',
			header : 'ID',
			width : 20,
	...
	url : {
		addUrl : basePath + "/admin/algorithms/addAlgorithms/8",
		detailUrl : basePath + "/admin/algorithms/getAlgorithmDetail",
		modifyUrl : basePath + "/admin/algorithms/modifyAlgorithms/8",
...
var gridPanel = CTA.common.GridPanel.createGridPanel(ROTEK.ALGORITHMS8.params.gridParam);
	...
	toolbar.get("button_add").setHandler(function() {
		var saveHandler = function() {
			...
			if (formPanel.getForm().isValid()) {
				formPanel.commit({
					url : ROTEK.ALGORITHMS8.params.url.addUrl
				});
			}
		};
		//定义添加的窗口
		...
		var ProjectStore = new Ext.data.Store({
			...
			proxy : new Ext.data.HttpProxy({
				url : ROTEK.ALGORITHMS8.params.url.listProejctUrl
			})
		})
...
if(toolbar.get("button_modify")){
	toolbar.get("button_modify").setHandler(function() {
		...
		Ext.Ajax.request({
			url : ROTEK.ALGORITHMS8.params.url.detailUrl,
			params : {
				id : id
			},
			success : function(response) {
				...
				var formPanel = new CTA.common.SFormPanel({
					items : [{
						...
						fieldLabel : '产水电导率设定值',
						emptyText : '请输入产水电导率设定值',
						name : 'mxt_csddlbz',
						value : data.mxt_csddlbz,
						minLength : 1,
						maxLength : 100
					}, {
						fieldLabel : '超额时间设定值',
				...
				var updateWindow = new CTA.common.UpdateWindow({
					...
					handler : function() {
						...
						if (formPanel.getForm().isValid()) {
							...
							formPanel.commit({
								url : ROTEK.ALGORITHMS8.params.url.modifyUrl
							});
						}
					}
				});
		...
		Ext.Msg.confirm("警告", "您确定删除这  " + ids.length + " 条数据吗？", function(button) {
			if ('yes' == button) {
				...
				CTA.common.Ajax.request({
					url : ROTEK.ALGORITHMS8.params.url.dropUrl,
					params : {
						ids : ids.toString()
					}
				});
