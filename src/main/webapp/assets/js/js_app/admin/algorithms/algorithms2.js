/** 设置算法 管理  */
Ext.ns("ROTEK.ALGORITHMS2");
ROTEK.ALGORITHMS2.params = {
	gridParam : {
		url : basePath + "/admin/algorithms/listAlgorithms/2",
		dataList : [ {
			index : 'id',
			header : 'ID',
			width : 20,
	...
	url : {
		addUrl : basePath + "/admin/algorithms/addAlgorithms/2",
		detailUrl : basePath + "/admin/algorithms/getAlgorithmDetail",
		modifyUrl : basePath + "/admin/algorithms/modifyAlgorithms/2",
...
var gridPanel = CTA.common.GridPanel.createGridPanel(ROTEK.ALGORITHMS2.params.gridParam);
	...
	toolbar.get("button_add").setHandler(function() {
		var saveHandler = function() {
			...
			if (formPanel.getForm().isValid()) {
				formPanel.commit({
					url : ROTEK.ALGORITHMS2.params.url.addUrl
				});
			}
		};
		//定义添加的窗口
		...
		var ProjectStore = new Ext.data.Store({
			...
			proxy : new Ext.data.HttpProxy({
				url : ROTEK.ALGORITHMS2.params.url.listProejctUrl
			})
		})
...
if(toolbar.get("button_modify")){
	toolbar.get("button_modify").setHandler(function() {
		...
		Ext.Ajax.request({
			url : ROTEK.ALGORITHMS2.params.url.detailUrl,
			params : {
				id : id
			},
				...
				var updateWindow = new CTA.common.UpdateWindow({
					...
					handler : function() {
						...
						if (formPanel.getForm().isValid()) {
							...
							formPanel.commit({
								url : ROTEK.ALGORITHMS2.params.url.modifyUrl
							});
						}
					}
				});
	...
	toolbar.get("button_algorithmDesc").setHandler(function() {
		var formPanel = new CTA.common.SFormPanel({
			items : [ {
				xtype : 'tbtext',
				readOnly : true,
				text : '1. 开关量的额定运行时间，单位是小时Na，Na=设置小时，固定值。<p>&nbsp;</p>'
				       + '2. 若当前时间记为Td，则距离第一条有效数据的时间差为累计运行时间Ha，数据库中保存为累计运行时间。<p>&nbsp;</p> '
			           + '3. Ha>Na时发送提醒。<p>&nbsp;</p> '
			           + '4. 计算公式：Ha > Na',
				height: 160
			}]
		...
		Ext.Msg.confirm("警告", "您确定删除这  " + ids.length + " 条数据吗？", function(button) {
			if ('yes' == button) {
				...
				CTA.common.Ajax.request({
					url : ROTEK.ALGORITHMS2.params.url.dropUrl,
					params : {
						ids : ids.toString()
					}
				});
