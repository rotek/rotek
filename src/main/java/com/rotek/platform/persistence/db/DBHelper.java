...
public class DBHelper {
	...
	*/
	public static void setPreparedParameters(PreparedStatement ps,
		...
		//check before
		if(parameterMetaData.getParameterCount() >0 && (null == parameters || parameters.length != parameterMetaData.getParameterCount())){
			throw new SQLException("parameters number is not correct");
		}
		if(null != parameters && parameters.length > 0){
			...
			for(int i = 1;i<=parameters.length;i++){
				...
				if(param instanceof String){
					...
					ps.setTimestamp(i,new Timestamp(((java.util.Date) param).getTime()));
				} else if (param instanceof Timestamp) {
					ps.setTimestamp(i, (Timestamp) param);
				} else if (param instanceof java.sql.Date) {
					ps.setTimestamp(i, new Timestamp(((java.sql.Date) param).getTime()));
					...
					ps.setTimestamp(i, new Timestamp(((java.sql.Time)param).getTime()));
				}else {
					throw new SQLException("SQLException","unknown data type : "+param);
				}
			}
		}
	}
	...
	* @throws
	*/
	public static Map<String, Object> getResultsetRow(ResultSet rs) throws SQLException {
	...
	*/
	public static Object getResultsetValue(ResultSet rs, int columnIndex) throws SQLException {
		...
		if("byte".equals(columnClassName) || "java.lang.Byte".equals(columnClassName)){
			return new Byte(rs.getByte(columnIndex));
		}else if("short".equals(columnClassName) || "java.lang.Short".equals(columnClassName)){
			return new Short(rs.getShort(columnIndex));
		}else if("int".equals(columnClassName) || "java.lang.Integer".equals(columnClassName)){
			return new Integer(rs.getInt(columnIndex));
		}else if("long".equals(columnClassName) || "java.lang.Long".equals(columnClassName)){
			return new Long(rs.getLong(columnIndex));
		}else if("double".equals(columnClassName) || "java.lang.Double".equals(columnClassName)){
			return new Double(rs.getDouble(columnIndex));
		}else if("float".equals(columnClassName) || "java.lang.Float".equals(columnClassName)){
		...
		}else if(LobType.CLOB_NAME.equals(resultSetMetaData.getColumnTypeName(columnIndex))){
			//获取Clob的值
			return getClobValue(rs,columnIndex);
		}else {
			throw new SQLException("Cannot Recognise the data Type, columnName="
				    + resultSetMetaData.getColumnName(columnIndex) + ", type=" + resultSetMetaData.getColumnClassName(columnIndex));
		}
	...
	* @throws
	*/
	public static String getClobValue(ResultSet rs, int columnIndex) throws SQLException {
		Reader reader = null;
		try{
			...
			if(null != reader){
				try {
					...
					e.printStackTrace();
					throw new SQLException("get clob error",e.getMessage());
				}
			}
		}
		return null;
	 ...
	 * @throws
	 */
	public static String getClobValue(ResultSet rs, String columnName) throws SQLException {
		Reader reader = null;
		try{
	...
	* @throws
	*/
	public static <T> String getInsertSql(EntityMapping entityMapping,T entity,List<Object> params,int dbType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		//前半部分sql
	...
	* @throws
	*/
	public static  <T> String getUpdateSql(EntityMapping entityMapping,T entity,List<Object> params,int dbType) throws SQLException, IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		Map<String,PropertyMapping> propertyMappings = entityMapping.getPropertyMapping();
		//前部分
		StringBuilder sql = new StringBuilder();
		//where
		...
		PropertyMapping idPropertyMapping = entityMapping.getIdPropertyMapping();
		sql_where.append(" where ");
		sql_where.append(idPropertyMapping.getPropertyName()+"=?");
		params.add(PropertyUtils.getProperty(entity, idPropertyMapping.getPropertyName()));
	 ...
	 * @throws
	 */
	public static  <T> String getDeleteSql(EntityMapping entityMapping,T entity,List<Object> params,int dbType) throws SQLException, IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		//SQL
		StringBuilder sql = new StringBuilder();
