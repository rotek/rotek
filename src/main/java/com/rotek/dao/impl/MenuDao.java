...
public class MenuDao extends BaseDaoImpl {
	...
	*/
	public List<Map<String, Object>> listMenus_super(String super_menu_ids) throws SQLException {
		...
		String sql = "select id,menu_name from r_menu where id in (select id from r_menu where super_menu_id in("+super_menu_ids+"))  and status = 1 order by super_menu_id,status,sort desc";
	...
	*/
	public List<Map<String, Object>> listMenus_sort() throws SQLException {
		...
		String sql = "select (sort-1) sort,menu_name from r_menu where status = 1 order by sort desc";
		return this.executeQuery(sql, null);
	}
	...
	*/
	public MenuDto getMenuDetail(Integer id) throws SQLException {
		String sql = "select id, menu_name, super_menu_id, url, sort, status from r_menu where id = ?";
